function varnum = practice(input,watermark,name,band_name,p)
    ...
    %embedding watermark
    embedded_LL = host_LL + ( ((d*0.55)/k) * watermark_LL);
    ...
    original_watermark_LL = (embedded_LL - host_LL)*(k/(d*0.55));
    ...
    X = sprintf('%10s\t %10s\t %5f\t %5f',name,band_name,PSNR,NCC);
    disp(X);
