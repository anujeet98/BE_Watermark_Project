function varnum = result_LH(input,watermark,name,band_name)
    ...
    %embedding watermark
    embedded_LH = host_LH + ( ((d*0.55)/k) * watermark_LH);
    embedded = idwt2(host_LL,embedded_LH,host_HL,host_HH,'haar');
    ...
    original_watermark_LH = (embedded_LH - host_LH)*(k/(d*0.55));
    extracted = idwt2(watermark_LL,original_watermark_LH,watermark_HL,watermark_HH,'haar');
    ...
    subplot(2,2,4),imshow(double(extracted));title("Extracted Image");
